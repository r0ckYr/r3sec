...
`, verificationLink, verificationLink)
	params := &resend.SendEmailRequest{
		...
		To:      []string{to},
		Html:    html,
		Subject: "Verify your r3sec email",
	}
...
func SendPasswordResetEmail(to string, token string) error {
	apiKey := os.Getenv("RESEND_API_KEY")
	from := os.Getenv("EMAIL_FROM") // e.g., R3SEC <no-reply@r3sec.xyz>
	client := resend.NewClient(apiKey)
	resetLink := fmt.Sprintf("https://r3sec.xyz/auth/reset-password?token=%s", token)
...
<div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; background-color: #0f0f0f; border: 1px solid #27272a; border-radius: 12px; padding: 40px 30px;">
  ...
  <h2 style="color: #ffffff; font-size: 24px; margin-top: 32px; margin-bottom: 20px;">Reset Your Password</h2>
  <p style="font-size: 16px; color: #e4e4e7; margin-bottom: 24px;">
    We received a request to reset your password for your <strong style="color: #22c55e;">R3SEC</strong> account. To create a new password, click the button below. This link will expire in 5 minutes.
  </p>
  <div style="text-align: center; margin: 30px 0;">
    <a href="%s" style="display: inline-block; background-color: #22c55e; color: #0f0f0f; padding: 14px 26px; font-size: 16px; font-weight: bold; border-radius: 8px; text-decoration: none;">
      Reset Password
  ...
  <p style="font-size: 13px; color: #71717a; margin-top: 32px;">
    If you did not request a password reset, please disregard this email or contact us immediately to secure your account.
  </p>
  <hr style="margin: 40px 0; border: none; border-top: 1px solid #27272a;" />
  <p style="font-size: 12px; color: #52525b; text-align: center;">
    &copy; 2025 R3SEC. All rights reserved.<br/>
...
`, resetLink, resetLink)
	...
	params := &resend.SendEmailRequest{
		...
		To:      []string{to},
		Html:    html,
		Subject: "Reset Your R3SEC Password",
	}
	
	sent, err := client.Emails.Send(params)
	if err != nil {
		fmt.Println("❌ Password reset email send error:", err.Error())
		return err
	}
	fmt.Println("✅ Password reset email sent. ID:", sent.Id)
	return nil
}
